...
def handle_msg():
    ...
    # 加入星座运势数据
    Cons_data = api.get_Cons()
    if Cons_data:
        Cons_tip = Cons_data["Cons_tip"]
        info_list.append(Cons_tip)

    # 加入一言数据
    hitokoto_data = api.get_hitokoto()
    if hitokoto_data:
        hitokoto_tip = hitokoto_data["hitokoto_tip"]
        info_list.append(hitokoto_tip)
